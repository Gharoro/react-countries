...
const countryService = {
  async getAllCountries() {
    try {
      const response = await axios.get(
        "https://restcountries.com/v2/all?fields=name,capital,currencies,tld,region,subregion,languages,flags,population"
      );
  ...
  async getCountry(country_name: string) {
    try {
      const response = await axios.get(
        `https://restcountries.com/v2//name/${country_name}?fields=name,capital,currencies,tld,region,subregion,languages,flags,population,topLevelDomain,nativeName,borders`
      );
